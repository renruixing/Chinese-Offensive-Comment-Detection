...
def Sent2Word(sentence):
    ...
    '''
    stop_words = [w.strip() for w in open(r'.\prepro_data\hit_stopwords.txt', 'r', encoding='UTF-8').readlines()]
    stop_words.append(' ')
    ...
    for word in phrase:
        if flag:
            ...
            if word in model.wv and word in vocabulary.keys():
                #result += model.wv.get_item(word) * vocabulary[word]
                result += model.wv[word] * vocabulary[word]
    return result
